...
| Day | Topic (Server for lab) | Resources |
| --- | --- | --- |
| 1 | Foundations and a full Visium workflow (https://learn.uke.de) | ~25 GB RAM|
...
Create the environment (default name is `env-spatial-course`):
...
- **Visium** (Day 1) and **seqFISH** (Day 3) download automatically the first time a
  notebook needs them.
- **Xenium** (Day 2) is available already in `/home/shared/spatial_course_data/xenium_kidney`
...
- Day 3 is deliberately small and self-contained so it fits 6 GB and does not depend
  on Days 1 to 2.
- GPU is available but not used by default. Methods that practically need a GPU (e.g. scVI, cell2location)
  are discussed in the slides, not trained live. If you want to train, feel free.
